...
- API Key: (stored in 1Password - SECRET - never commit)
- Models: claude-sonnet-4-5-20250929 (chat), claude-haiku-4-5-20251001 (intent classification)
- Used for: WF42 Slack bot AI chat, voice AI responses, intent classification

**OPENAI:**
- API Key: (stored in 1Password - SECRET)
- Used for: Whisper voice transcription ONLY (WF42)
...
2. **Workflow Naming & Organization**
   ...
   - **Sticky Note Documentation Standard (MANDATORY):**
     - Every workflow MUST have sticky notes explaining what each section does
     - Language: Simple, plain English (8th grade reading level) — no jargon
     - One sticky note per logical section (group of related nodes)
     - Each sticky note includes: section name, what it does, and the flow
     - Sticky notes must be positioned above/near the nodes they describe
     - When building a NEW workflow: add sticky notes during initial build
     - When modifying an EXISTING workflow: update sticky notes if the section changes
     - Sticky note node type: `n8n-nodes-base.stickyNote` (typeVersion 1)
     - Minimum width: 300, minimum height: 120
     - Example content format:
       ```
       ## Section Name
       What happens here: [plain English description]
       Nodes: [list of nodes in this section]
       ```
...
│  ──────────────────           ─────────────               ──────────────    │
│  WF200 Task Tracker           WF201 Sprint Update         WF204 Health Check│
...
| WF201 | 12 hour Dev Sprint update | 12-hour sprint update sent to Slack | HIGH |
...
🔄 IN PROGRESS
   ...
   • WF201 12 hour Dev Sprint update — this workflow, testing now
...
- WF201 12 hour Dev Sprint update deployed
...
6:00 PM  — 12 hour Dev Sprint update arrives (WF201)
...
8. Deploy WF201 (12 hour Dev Sprint update)
...
- [x] WF201 v1.3 — 12 hour Dev Sprint update (Day 1)
...
## 16. HIPAA/PCI Compliance Framework

**Last Updated:** 2026-02-07
**Status:** P0 tables deployed, framework active
**Voice Provider Note:** All references to voice AI are provider-agnostic. The same BAA, recording, and transcript rules apply regardless of vendor (Vapi, Bland, ElevenLabs, Retell, or other).

### 16.1 Core Principle

**HIPAA does NOT say "don't store patient data." It says "if you store patient data, protect it properly."**

RevOS stores names, phones, emails, DOBs, insurance info, call recordings, transcripts, and behavioral memories. This is 100% legal and necessary for the business to function. The compliance work is about HOW we protect it — not WHETHER we collect it.

RevOS is a **Business Associate** under HIPAA. Every customer practice is a **Covered Entity**. We must have a signed BAA with each practice and with every vendor that touches PHI.

### 16.2 Data Classification (Four Tiers)

Every field in every table is classified. This drives encryption, access, redaction, and retention.

| Tier | Label | Examples | Encryption | Redact in External Logs? | Retention |
|------|-------|----------|------------|--------------------------|-----------|
| **T1** | Public | tenant name, office hours, provider names | Supabase default | No | Indefinite |
| **T2** | Operational | appointment status, workflow logs, task lists | Supabase default | No | 6 years |
| **T3** | PHI/PII | patient name, phone, email, DOB, address, insurance ID, transcripts, memories, recordings | Supabase default + column-level for transcripts | **Yes** — mask in Slack/external | 7 years |
| **T4** | Secrets | API keys, tokens, passwords, SSN, raw credit card numbers | Application-level encryption | **Full redact** `[REDACTED]` | Never stored (tokenize) |

**Classification registry**: `data_classification` table — metadata for every T3+ column.

**Why NOT encrypt every field**: Column-level encryption prevents SQL queries (can't `WHERE` on encrypted data). Standard approach (used by Epic, Athena, Dentrix): protect with RLS + RBAC + audit logging. Only encrypt high-value fields rarely queried by content (transcripts, insurance verification details).

### 16.3 What We Store & Why (Maximizing Data Utility)

| Data | Where Stored | Why We Need It | Tier |
|------|-------------|----------------|------|
| Patient name | patients table (future) | Personalization, scheduling, greeting by name | T3 |
| Phone number | phone_calls.from_number | Identity resolution, callback, SMS | T3 |
| Email | patients table (future) | Communication preference, reminders | T3 |
| Date of birth | patients table (future) | Insurance verification, identity | T3 |
| Address | patients table (future) | Insurance claims, billing | T3 |
| Insurance carrier + member ID | patient_insurance | Eligibility verification | T3 |
| Call recordings | Supabase Storage (private bucket) | Quality review, dispute resolution, training | T3 |
| Transcripts | call_sessions.transcript | Memory extraction, summary generation | T3 |
| Behavioral memories | memories.content | "Prefers Dr. Chen", "interested in Invisalign" — THE MOAT | T3 |
| Appointment history | appointments | Scheduling, follow-up, no-show tracking | T3 |
| Communication preferences | memories | "Prefers text over call", "best after 5pm" | T3 |
| Credit card numbers | **NEVER stored** — Stripe tokenizes | Billing (via Stripe token only) | T4 |
| API keys, tokens | n8n credentials (1Password) | System operation | T4 |

### 16.4 Credit Card Handling (PCI-DSS)

**Rule: RevOS NEVER stores raw credit card numbers.** Period.

```
Patient says card number on call
  → AI says "Let me process that for you"
  → Recording pauses (configurable per voice provider)
  → Card sent directly to Stripe → Stripe returns token + last 4
  → RevOS stores: stripe_token, last_four, card_brand, exp_month, exp_year
  → Recording resumes
  → Full card number NEVER touches our database
```

Storage: `payment_methods` table (stripe_customer_id, stripe_payment_method_id, last_four, card_brand, exp_month, exp_year). This keeps RevOS completely out of PCI scope (SAQ-A level).

### 16.5 Insurance Information

Standard PHI storage in `patient_insurance` table. Collected over the phone — AI asks for carrier name, member ID, group number, subscriber info. Protected by RLS + audit logging. Same protection as any other T3 data.

### 16.6 Consent Management

**Table**: `consent_records` — tracks patient consent by type.
**Function**: `check_consent(tenant_id, patient_id, consent_type)` → returns boolean.

| Consent Type | When Checked | Default |
|-------------|-------------|---------|
| `voice_recording` | Start of every call | Verbal notice ("This call may be recorded") — one-party consent states |
| `sms_communication` | Before outbound SMS | Must opt in (TCPA) |
| `email_communication` | Before marketing email | Must opt in (CAN-SPAM) |
| `ai_processing` | First call / intake | Covered by ToS + verbal notice |
| `data_sharing` | Before sharing with third party | Must explicitly authorize (HIPAA §164.508) |

**Two-party consent states** (CA, CT, FL, IL, MD, MA, MT, NH, PA, WA): AI must ask for explicit consent. `consent_rules` stored per tenant based on state.

### 16.7 Redaction Rules

**WF11 (Event Logger)**: Strips T4 data (secrets/tokens/credentials) before database write. Patient data (T3) is NOT redacted in the database — the database IS the system of record. Auditors need full data.

**WF15 (Redactor — future)**: Masks T3 data before it leaves RevOS:

| Data Type | Redacted Output |
|-----------|-----------------|
| Phone | +1(555)\*\*\*-4567 |
| Email | s\*\*\*\*@g\*\*\*\*\*.com |
| Name | S. Johnson |
| DOB | \*\*/\*\*/1985 |
| Insurance ID | DEN\*\*\*\*\*6789 |
| Transcript | [TRANSCRIPT - 847 words] |

**Applied to**: Slack alerts, error logs, external monitoring, n8n execution logs.
**NOT applied to**: Database writes, internal workflow data, admin API responses, audit log entries.

### 16.8 Access Control (RBAC)

**Table**: `role_permissions` — per-tenant permission matrix.

| Role | patients | appointments | billing | reports | settings | recordings |
|------|----------|-------------|---------|---------|----------|------------|
| admin | RWD | RWD | RWD | R | RW | R |
| staff | R | RW | — | R | — | R |
| billing | R (name only) | R | RW | R | — | — |
| readonly | — | R | — | R | — | — |

**PHI Access Logging**: Every patient data view/export/update logged as `phi_access` event in WF11 with: action, fields_accessed, justification, actor_id, ip_address.

### 16.9 BAA Requirements

| Vendor | Service | BAA Status | Data Shared |
|--------|---------|------------|-------------|
| Supabase | Database | **SIGN** (Pro plan, self-service) | All PHI |
| Twilio | Voice/SMS | **SIGN** (HIPAA plan) | Phone numbers, recordings, SMS |
| Voice AI Provider | AI Voice | **VERIFY** (selection criterion) | Transcripts, context, recordings |
| OpenAI | Whisper | **SIGN** (API BAA) | Audio files |
| Anthropic | Claude API | **SIGN** (enterprise BAA) | Conversation context |
| Stripe | Payments | **NOT REQUIRED** | Tokens only |
| GitHub | Code repo | **NOT REQUIRED** | No PHI in repo |
| Slack | Internal alerts | **EVALUATE** | Redacted only (if WF15 deployed) |

**All BAAs must be signed before deploying patient-facing workflows (WF16/WF17).**

### 16.10 Data Retention

| Data Type | Retention | Auto-Action |
|-----------|-----------|-------------|
| Call recordings | 7 years | Archive to cold storage after 1yr |
| Transcripts | 7 years | Same |
| Audit logs (events) | 6 years (HIPAA §164.316) | Archive after 2yr |
| Patient contact info | Until deletion request + 30 days | Manual |
| Behavioral memories | Until deletion request | Manual |
| Insurance info | 7 years from last service | Auto-archive |
| Payment tokens | Until patient removes card | Stripe manages |
| Workflow execution logs | 90 days | Auto-delete |

### 16.11 Patient Rights (Build before 100 customers)

| Right | HIPAA Section | Response Time |
|-------|--------------|---------------|
| Access ("Give me my data") | §164.524 | 30 days |
| Amendment ("Fix my data") | §164.526 | 60 days |
| Accounting of Disclosures | §164.528 | 60 days |
| Restriction | §164.522 | 30 days |
| Deletion (CCPA) | CCPA §1798.105 | 45 days |

### 16.12 Breach Response

**Detection** (build into WF84 Compliance Sentinel):
- Alert if user accesses >50 patient records in 1 hour
- Alert if data exported
- Alert if RLS policy dropped/modified
- Alert if failed auth attempts >10 in 5 minutes

**Response runbook**:
1. **Contain (0-15 min)**: Disable compromised credential/account, preserve evidence
2. **Investigate (15 min-24 hr)**: Query events table, determine scope
3. **Notify (24 hr-60 days)**: HHS notification (60 days), patient notification, media if ≥500 patients

### 16.13 Voice Recording Rules

Every call starts with disclosure:
```
"Thank you for calling [Practice Name]. This call may be recorded for quality
and training purposes. How can I help you today?"
```

Two-party consent states add: "Do you consent to this call being recorded?"

**Storage**: Download from voice provider → upload to Supabase Storage private bucket (`/{tenant_id}/{year}/{month}/{call_id}.wav`). Delete from provider after confirmation.

### 16.14 Implementation Priority

| Priority | What | When |
|----------|------|------|
| **P0** | RLS on all tables, data_classification table, consent_records table, vendor_compliance table, payment_methods table, patient_insurance table, WF11 phi_access support | Before patient data enters system |
| **P1** | role_permissions table, WF15 Redactor, recording storage bucket, breach runbook, privacy policy | Before first paying customer |
| **P2** | Patient rights workflows, WF84 Compliance Sentinel, automated retention/archival, external security audit | Before 100 customers |

### 16.15 What We Explicitly Do NOT Store

- Clinical notes (read-only from EHR)
- Diagnoses or treatment plans
- Prescriptions or lab results
- Raw credit card numbers (Stripe tokenizes)
- Any data from one tenant visible to another (RLS enforced)
